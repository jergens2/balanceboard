...
import { DaybookSleepCycle } from "./daybook-sleep-cycle.class";
...
export class SleepCycleBuilder {
    ...
    buildSleepCycleForDate(dateYYYYMMDD: string, dayItems: DaybookDayItem[], appConfig: UAPAppConfiguration): DaybookSleepCycle {
        ...
        const newSleepCycle = new DaybookSleepCycle(dateYYYYMMDD, dayItems, appConfig,
    ...
    private _findPreviousFallAsleepTime(dateYYYYMMDD: string, dayItems: DaybookDayItem[]): moment.Moment {
        if(dayItems[0].hasSleepItems && dayItems[1].hasSleepItems){
            ...
            const schedItems = this._getSchedItems(dateYYYYMMDD, dayItems);
            ...
            let fallAsleepTime = moment(dateYYYYMMDD).startOf('day').subtract(24, 'hours').add(addHours, 'hours').add(addMins, 'minutes');
    ...
    private _findNextFallAsleepTime(dateYYYYMMDD: string, dayItems: DaybookDayItem[]): moment.Moment {
        if(dayItems[1].hasSleepItems && dayItems[2].hasSleepItems){
            ...
            const addHours = this._appConfig.defaultFallAsleepHour;
            const addMins = this._appConfig.defaultFallAsleepMinute;
            let fallAsleepTime = moment(dateYYYYMMDD).startOf('day').add(addHours, 'hours').add(addMins, 'minutes');
            return fallAsleepTime;
        }
    }
    private _findNextWakeupTime(dateYYYYMMDD: string, dayItems: DaybookDayItem[]): moment.Moment {
        if(dayItems[1].hasSleepItems && dayItems[2].hasSleepItems){
            ...
            const addHours = this._appConfig.defaultWakeupHour;
            const addMins = this._appConfig.defaultWakeupMinute;
            let wakeupTime = moment(dateYYYYMMDD).startOf('day').add(24, 'hours').add(addHours, 'hours').add(addMins, 'minutes');
            return wakeupTime;
        }
    }

    private _getSchedItems(dateYYYYMMDD: string, dayItems: DaybookDayItem[]) {
