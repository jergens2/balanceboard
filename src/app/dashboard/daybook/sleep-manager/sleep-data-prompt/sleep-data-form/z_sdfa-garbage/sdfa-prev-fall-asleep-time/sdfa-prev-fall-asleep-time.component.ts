...
@Component({
  selector: 'app-sdfa-prev-fall-asleep-time',
  templateUrl: './sdfa-prev-fall-asleep-time.component.html',
  styleUrls: ['./sdfa-prev-fall-asleep-time.component.css']
})
export class SdfaPrevFallAsleepTimeComponent implements OnInit {
  ...
  ngOnInit(): void {
    this._wokeUpAt = moment(this.sleepService.sleepDataForm.formInputWakeupTime);
    if (this.sleepService.sleepDataForm.prevFallAsleepTimeIsSet) {
      this._fallAsleepTime = moment(this.sleepService.sleepDataForm.formInputPrevFallAsleep);
    } else {
      this._fallAsleepTime = moment(this.sleepService.previousFallAsleepTime);
    }
    this._setMinValue();
    this._setMaxValue();
  ...
  onTimeChanged(time: moment.Moment) {
    ...
    this._sleepDuration = DurationString.calculateDurationString(this._fallAsleepTime, this.wokeUpAt)
    this.sleepService.sleepDataForm.setformInputPrevFallAsleep(moment(time));
  }
