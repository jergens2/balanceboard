...
@Component({
  selector: 'app-sdfa-dreams',
  templateUrl: './sdfa-dreams.component.html',
  styleUrls: ['./sdfa-dreams.component.css']
})
export class SdfaDreamsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.sleepService.sleepDataForm.dreamsSet) {
      if (this.sleepService.sleepDataForm.formInputDreams.length > 0) {
        this._existingDreams = this.sleepService.sleepDataForm.formInputDreams.filter(item => true);
        this._displayDreams = this.sleepService.sleepDataForm.formInputDreams.filter(item => true);
      }
    } else {
  ...
  private _resub() {
    ...
    this._valueChangeSub = this.dreamsForm.controls['dreams'].valueChanges.subscribe((valueChange) => {
      if (valueChange !== null) {
        ...
        this.sleepService.sleepDataForm.setformInputDreams(this._existingDreams.filter(item => true));
